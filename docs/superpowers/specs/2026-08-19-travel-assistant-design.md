...
| POI 数据 | **RAG 知识库**（全国约 20 城景点/酒店/餐厅，LLM 自举生成语料 + BGE + Chroma） | 唯一数据源；语义检索；展示完整 RAG 管线；语料标注"AI 生成示例数据" |
...
| Agent | 职责 | 工具 |
|---|---|---|
| Supervisor 主管 | 意图识别（规划 / 修改请求），分派任务，汇总输出 | — |
| Analyst 需求分析师 | 追问补全出行信息，构建用户画像 | 会话记忆 |
| Planner 规划师 | 天级行程生成 + 修改后局部重排（吸收 Reviser：修订=带修改指令的重新规划） | 各 Agent 产出 |
| Researcher 研究员 | 景点/酒店/餐厅检索推荐（RAG 语义检索）、实时天气 | `poi_search`（Chroma+BGE） / `weather` |
| Budget 预算官 | 按预算上限分配住宿/交通/餐饮/门票，产出预算表 | 预算模型 |

### 协作流程示例

1. *"10月去成都玩3天，预算8000，喜欢美食"* → Analyst 补全 → Researcher（RAG 检索景点/酒店/餐厅 + 天气并行 fan-out）→ Budget 分配 → Planner 生成 → Supervisor 汇总输出「行程 + 预算表 + 天气提示」
...
│   │   ├── rag/                  # embeddings / vector_store / retriever / ingest / generate
...
- **RAG POI 语料自举**：DeepSeek 按城市模板批量生成景点/酒店/餐厅条目（约 20 城）→ 校验 → BGE embedding → Chroma 入库；人工抽查润色；**"周边"= 同城 + 经纬度距离排序**；语料标注"AI 生成示例数据，坐标仅供参考"
...
3. 实时天气（影响行程：雨天排室内）
4. 个性化推荐 + 行程修改重排
5. POI 详情查询与**周边推荐**（景点周边的酒店/餐厅，按距离排序；时间线与地图双向联动）
6. 地图可视化
...
| M2 最小闭环 | Provider、**RAG POI 知识库（约 20 城）**、Analyst+Planner、天气适配器 | 国内城市简单行程可用，RAG 检索命中 |
...
- M2：RAG 检索命中验证（"成都 火锅"命中火锅店；语料入库 count ≥ 300）
